...
const onSubmitForm = (event) => {
    ...
    if (formState === "CREATE") {
        addNewEmployeeRecord(employee);
          ...
          // Loop through employeesList to find the employee to update
          for (let i = 0; i < employeesList.length; i++) {
            if (employeesList[i].employeeId === employee.employeeId) {
                // Update the employee record
                employeesList[i].name = employee.name;
                employeesList[i].salary = employee.salary;
                employeesList[i].role = employee.role;
                employeesList[i].team = employee.team;
                employeesList[i].companyName = employee.companyName;

                // Find the corresponding table row and update its cells
                const record = recordsContainer.querySelector(`tr[data-empId="${employee.employeeId}"]`);
                const cells = record.querySelectorAll('td');
                cells[0].innerText = employee.name;
                cells[1].innerText = employee.salary;
                cells[2].innerText = employee.role;
                cells[3].innerText = employee.team;
                cells[4].innerText = employee.companyName;
